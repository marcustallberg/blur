...
var blur = new Blur({pixelSize:15,dropAreaId:"dropArea"})
//bind polyfill
...
Blur.prototype.uploadToimgurl = function(e){
    ...
    try {
        var img = this.targetCanvas.toDataURL('image/jpeg', 0.9).split(',')[1];
    } catch(e) {
        var img = this.targetCanvas.toDataURL().split(',')[1];
    }
    var w = window.open();
    ...
    $.ajax({
        ...
        data: {
            ...
            caption: 'Pixelate your images in your broser',
            image: img
        },
        dataType: 'json'
...
Blur.prototype.blurAll = function(e) {
    ...
    var loopAmount = ref.pixelSize * ref.pixelSize * 4; 
    //cache image data
    if(ref.imageData!==null ){
        ...
        var imgd = ref.context.getImageData(0,0,imageWidth,imageHeight);
...
Blur.prototype.blurFill = function(pixelObj,onepass) {  
    ...
    switch(this.blurStyle){
        ...
        case "invertedBackdrop":
            var rgbValueInverted = this.getRgbValue(r,g,b,loopAmount,true);
...
function setPlacement(){
    ...
    if($(blur.targetCanvas).is(":hidden")){
...
$("#mouseBlur, #scaleImage").change(
    function(e) {
        ...
        if($(e.currentTarget).attr("id")==="mouseBlur"){
            blur.blurWithMouse($(e.currentTarget).is(':checked'));
        }
        else{
            blur.scaleToFit($(e.currentTarget).is(':checked'));
        }
    }
);
